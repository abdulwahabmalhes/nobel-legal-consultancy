...
import { ArrowRight, BadgeCheck, BriefcaseBusiness, Building2, Clock3, Globe2, Handshake, ShieldCheck } from "lucide-react";
...
export default function HomePage() {
  return (
    <>
      <section className="relative flex min-h-screen items-center overflow-hidden bg-[#171012] pt-24 text-white">
        <Image src="/images/hero-enhanced.png" alt="NOBEL Legal Consultancy reception" fill priority sizes="100vw" className="object-cover" />
        <div className="absolute inset-0 bg-[linear-gradient(90deg,rgba(15,8,10,.95),rgba(18,9,11,.76)_45%,rgba(18,9,11,.28))]" />
        ...
        <div className="section-shell relative py-24">
          <Reveal className="max-w-3xl">
            ...
            <h1 className="font-display mt-7 text-balance text-5xl leading-[1.02] sm:text-6xl lg:text-[5.2rem]">Strategic legal counsel for <span className="silver-text italic">decisive moments.</span></h1>
            ...
            <div className="mt-9 flex flex-col gap-3 sm:flex-row"><Button asChild size="lg"><Link href="/contact#consultation">Book a Consultation <ArrowRight className="size-4" /></Link></Button><Button asChild size="lg" variant="outline"><Link href="/services">Explore Expertise</Link></Button></div>
          </Reveal>
        </div>
      </section>
        ...
        <div className="section-shell grid gap-6 lg:grid-cols-2">
          ...
          <div className="bg-[#eeeae6] p-8 md:p-14">
            ...
            <div className="mt-10 grid gap-7 sm:grid-cols-2">
              {[[Globe2, "UAE Market Expertise"], [ShieldCheck, "Professional Integrity"], [Clock3, "Responsive Guidance"], [Handshake, "Long-Term Partnership"]].map(([Icon, title]) => <div key={title as string}><Icon className="size-6 text-[#6a001a]" strokeWidth={1.4} /><h3 className="font-display mt-4 text-lg">{title as string}</h3><p className="mt-2 text-sm leading-6 text-black/48">Clear legal thinking grounded in real commercial context.</p></div>)}
            </div>
          </div>
        </div>
      </section>

      <section className="bg-[#650018] py-24 text-white">
