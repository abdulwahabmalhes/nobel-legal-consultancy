...
export default function ContactPage() {
  ...
  return (
    <>
      ...
      <section className="py-24 md:py-32"><div className="section-shell grid gap-12 lg:grid-cols-[.8fr_1.2fr]"><div><SectionTitle eyebrow="Our Office" title="Professional guidance, within reach." /><div className="mt-10 space-y-7"><ContactLine icon={MapPin} label="Office Location" value="SPC Free Zone - Sharjah, UAE" /><ContactLine icon={Phone} label="Phone" value="+971 55 227 0676" href="tel:+971552270676" /><ContactLine icon={Mail} label="Email" value="info@nobellegal.ae" href="mailto:info@nobellegal.ae" /><ContactLine icon={Clock3} label="Working Hours" value="Monday - Friday, 8:00 AM - 6:00 PM" /></div><div className="mt-10 flex flex-wrap gap-3"><Button asChild><a href="tel:+971552270676"><Phone className="size-4" /> Call</a></Button><Button asChild variant="ghost" className="border border-black/10"><a href="https://wa.me/971552270676"><MessageCircle className="size-4" /> WhatsApp</a></Button></div></div><div id="consultation" className="scroll-mt-28 bg-[#eeeae6] p-6 md:p-10"><h2 className="font-display text-3xl">Book a Consultation</h2><p className="mt-3 text-sm leading-7 text-black/50">Provide a brief outline and your preferred date. We will contact you to confirm availability.</p><form onSubmit={submit} className="mt-8 grid gap-4 md:grid-cols-2"><label className="form-label">Full name<input required className="form-field" /></label><label className="form-label">Phone number<input required type="tel" className="form-field" placeholder="+971" /></label><label className="form-label">Email address<input required type="email" className="form-field" /></label><label className="form-label">Preferred date<input required type="date" className="form-field" /></label><label className="form-label md:col-span-2">Area of support<select required defaultValue="" className="form-field"><option value="" disabled>Select a service</option>{services.map((service) => <option key={service.title}>{service.title}</option>)}</select></label><label className="form-label md:col-span-2">Brief summary<textarea rows={5} className="form-field resize-none py-4" /></label><div className="md:col-span-2"><Button type="submit" size="lg">Request Consultation <CalendarDays className="size-4" /></Button></div>{sent && <p className="flex items-center gap-2 text-sm font-semibold text-[#6a001a] md:col-span-2"><Check className="size-4" /> Thank you. Your request has been received.</p>}</form></div></div></section>
...
function ContactLine({ icon: Icon, label, value, href }: { icon: React.ElementType; label: string; value: string; href?: string }) {
  const content = <><Icon className="mt-1 size-6 text-[#6a001a]" strokeWidth={1.4} /><div><p className="text-[.62rem] font-bold tracking-[.15em] text-black/35">{label.toUpperCase()}</p><p className="mt-2 text-sm font-semibold">{value}</p></div></>;
  return href ? <a href={href} className="flex gap-4 hover:text-[#6a001a]">{content}</a> : <div className="flex gap-4">{content}</div>;
}
