...
export default function ServicesPage() {
  return (
    <>
      ...
      <section className="bg-[#eeeae6] py-24 md:py-32"><div className="section-shell grid gap-12 lg:grid-cols-2 lg:items-center"><ImagePanel src="/images/office-team.jpeg" alt="NOBEL legal team" className="aspect-[4/3]" /><div><SectionTitle eyebrow="How We Work" title="From legal question to controlled action." /><div className="mt-9 space-y-6">{["Understand the facts, objective, and commercial context.", "Assess legal exposure, documents, and available options.", "Present a clear strategy with priorities and practical next steps.", "Support implementation and remain available as matters evolve."].map((item, index) => <div key={item} className="flex gap-4"><CheckCircle2 className="mt-1 size-5 shrink-0 text-[#4a0012]" /><div><p className="text-xs font-bold tracking-[.12em] text-black/35">STEP {index + 1}</p><p className="mt-1 text-sm leading-7 text-black/60">{item}</p></div></div>)}</div></div></div></section>
      <CtaBand />
    </>
  );
}
